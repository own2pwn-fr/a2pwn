"""Verification oracle for SSTI.

Strongest-first: a ``differential`` oracle proving arithmetic evaluation (the
literal `7*7` renders to `49` / an engine-specific product — a string cannot
become its product without server-side evaluation), then a ``signature`` match of
the rendered value or RCE output, and an ``oob`` callback for blind engine
gadgets. Exposes ``verify(ctx)`` for the adversarial verifier, which builds the
class's :class:`~a2pwn.oracles.VerificationOracle` and delegates to
...
ORACLE_PRIORITY: tuple[str, ...] = ("differential", "signature", "oob", "marker")
SIGNALS: list[str] = ["49", "uid=", "gid=", "root:x:0:0", "1337", "7777777", "9359"]
EXPECT: dict[str, dict] = {"differential": {"signal": "reflection", "marker": "49"}}
